...
describe('Persistent Node Chat Server', () => {
    ...
    axios.post(`${API_URL}/users`, { username })
      ...
      .then(() => {
        ...
        dbConnection.query(queryString, queryArgs, (err, results) => {
          ...
          expect(results[0].text_message).toEqual(message);
          done();
        });
      })
      .catch((err) => {
  ...
  it('Should output all messages from the DB', (done) => {
    ...
    const queryString = 'INSERT INTO messages values (00001,test,00002,00001)';
    const queryArgs = [];
    ...
    dbConnection.query(queryString, queryArgs, (err) => {
      ...
      axios.get(`${API_URL}/messages`)
        .then((response) => {
          ...
          expect(messageLog[0].text_message).toEqual(message);
          expect(messageLog[0].room_id).toEqual(roomname);
          done();
        })
        .catch((err) => {
          throw err;
